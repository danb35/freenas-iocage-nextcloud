...
NextcloudBR.sh and NextcloudBR-config are automatically copied to the jails /usr directory. At the end of the restore the script will reset the admin password and ask for a new one. You can automate backup up by setting the cron variable to "yes" in the NextcloudBR-config. It will then automatically backup and not give you the option to restore. The version of nextcloud must be the same as the version of the backup or it will break the install. A log file will be created /usr/crontab.log.
...
```
45 3 * * * /usr/NextcloudBR.sh > /usr/crontab.log
```
